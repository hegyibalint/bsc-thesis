...
The guarantees most system can give are soft deadlines, meaning the system provides best effort, but does not guarantee real hard time guarantees. There are hard real-time operating systems like \textls{VxWorks} or \textls{FreeRTOS}.

The BeagleBone Black uses a 4.1.x \textls{RT}-Preempt patched kernel, which means it can serve hard real-time functions, but still the operating system and software running on them should considered as non-reliable components. To bypass the operating system, one can use the \pru{}s embedded inside of the AM335x processor core.
...
The optimization level of \texttt{O1}, \texttt{O2}, and \texttt{O3} halves the code size, but there is no major code size reduction above \texttt{O1}. Level \texttt{O2}, \texttt{O3}, and \texttt{O4} have practically the same code size.
...
Apart from host side performance, the poll based communication illustrate how big is the penalty of a \textls{DDR3} memory access. The test automation used in the measurements was the biggest which could be fitted inside the \pru{} instruction memory with $7 \cdot 2^7 = 896$ transitions total, $128$ transition possibility per event. The stepping of the automaton was handle under \si{1}{\mics}, therefore the automaton limited much by size of the automaton rather then the execution time of it.
