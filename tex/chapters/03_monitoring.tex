...
\begin{figure}
	...
	\begin{tabular}{l l l}
		...
		A (A \& B) & Encapsulating the matches inside parentheses & AAB or ABA \\
		A+ & At least once, captures all same events & A\ldots \\
		A* & Captures all same events & AAAA\ldots \\
		{A}<x> & The encapsulated expression should match in x time units & \\
		\bottomrule
	\end{tabular}
	\label{fig:regex}
\end{figure}
